...
def create_new_donation_sheet(event_name):
    ...
    try:
        ...
        columns = ["ID","Name", "Email", "Phone","TRX ID","EVENT","Registration Date"]
...
def append_to_volunteer_sheet(file_id, data):
    try:
        ...
        headers = ["ID","Name", "Email", "Phone","TRX ID","EVENT","Registration Date"]
        try:
        ...
        new_row = [
            ...
            data.get('phone', ''),
            data.get('trx_id', ''),
            data.get('event', ''),  # You can customize this to match your event name column 
            current_time
        ]
